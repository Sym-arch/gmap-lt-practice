...
/* 先着◯名・初月無料キャンペーンの上限 */
export const CAMPAIGN_FREE_LIMIT = 100;
/* 無料期間（日） */
export const CAMPAIGN_TRIAL_DAYS = 30;

/* これまでに初月無料（トライアル）で成立した契約数。
   source='campaign' の件数で数える（トライアル終了後も残る恒久マーカー）。 */
export async function countCampaignSubscriptions() {
...
export async function saveSubscriptionFromSession(session, stripe, userId) {
  ...
  // trial_end がある＝初月無料キャンペーン → monitor プラン
  const isCampaign = !!sub.trial_end;
  const { error } = await svc.from("subscriptions").upsert(
    {
      ...
      source: isCampaign ? "campaign" : "stripe",
      plan:   isCampaign ? "monitor"  : "normal",
